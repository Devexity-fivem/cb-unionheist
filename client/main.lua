            ...
            onSelect = function()
                ...
                if robbed then
                    exports.ox_lib:notify({
                        ...
                        description = "This guard has already been robbed!",
                        type = "error"
                    })
                else
                    RobCopAnim(ped, k)
...
function BlowVaultDoor()
    if LocalPlayer.state.UnionHeist and not doingSomething then
        ...
        if removedItem then
            ...
            exports.ox_lib:notify({
                title = "Missing Item",
                description = "You are missing the required item",
                type = "error"
            })
        end
    else
        exports.ox_lib:notify({
            title = "Headaches",
            description = "You need to exit the depository and re-enter!",
            type = "error"
        })
    end
end
...
function SecurityHackAnim(hack)
    if LocalPlayer.state.UnionHeist and not doingSomething then
        -- Check police count
        local enoughPolice = lib.callback.await('cb-unionheist:server:CheckPoliceCount', false)
        if not enoughPolice then
            exports.ox_lib:notify({
                title = "Not Enough Police",
                description = "There aren't enough police online to start the heist!",
                type = "error"
            })
            return
        end
...
TeleportInside = lib.zones.box({
    coords = vector3(Config.TeleportIn.x, Config.TeleportIn.y, Config.TeleportIn.z),
    size = vec3(2, 1, 1),
    rotation = Config.TeleportIn.w,
    debug = false,
    inside = function()
...
TeleportOutside = lib.zones.box({
    coords = vector3(Config.TeleportOut.x, Config.TeleportOut.y, Config.TeleportOut.z),
    size = vec3(2, 1, 1),
    rotation = Config.TeleportOut.w,
    debug = false,
    inside = function()
                ...
                options = {
                    {
                        ...
                        canInteract = function()
                            -- Optional: Uncomment to check police count before showing interaction
                            -- Note: This may impact performance due to frequent server callbacks
                            -- local enoughPolice = lib.callback.await('cb-unionheist:server:CheckPoliceCount', false)
                            -- return enoughPolice and HasItemClient(Config.HackLocations[k].required.item, Config.HackLocations[k].required.amount) and not doingSomething
                    ...
                    onSelect = function()
                        ...
                        if securityHacked then
                            ...
                            exports.ox_lib:notify({
                                ...
                                description = "Take out the security system first!",
                                type = "error"
                            })
                        end
                    end,
                ...
                options = {
                    {
                        ...
                        onSelect = function()
                            if vaultBlown then
                                ...
                                exports.ox_lib:notify({
                                    title = "Blow Vault",
                                    description = "Blow the Vault Door first",
                                    type = "error"
                                })
                            end
                        end,
